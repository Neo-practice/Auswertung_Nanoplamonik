...
# 3 Peaks
delta = 13
for i, filename in enumerate([r"3peaks/22.csv", r"3peaks/21_02.csv", r"3peaks/01.csv"]):
    ...
    ax.set_xlim(441, 765)
    ax.set_ylim(-0.1, 1.1)
    ax.text(-0.1, 1.1, string.ascii_uppercase[i], transform=ax.transAxes, size=12, weight='bold')

    ax.plot(wavelength, spec/a, label="_Messdaten", color="#969696")
    ax.plot(wavelength, fit / a, label="_Summe aller Fits", color="black")
...
for i, filename in enumerate([r"2peaks/14_02.csv"]):
    ...
    ax.set_ylim(-0.1, 1.1)
    ax.yaxis.set_ticklabels([])
    ax.text(-0.1, 1.1, string.ascii_uppercase[3], transform=ax.transAxes, size=12, weight='bold')
...
fig.legend(bbox_to_anchor=(0.01, 0, 0.98, 0.1), loc="lower left", mode="expand", bbox_transform=fig.transFigure, ncol=3)

plt.show()
fig.savefig(r"plots/4plots_02.png")
#plt.close("all")
